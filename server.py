...
def analyze_demand_trends(data):
    ...
    # Get non-zero values for better analysis
    ...
    # Calculate recent growth rate using last 3 months
...
def predict():
    try:
        ...
        for medicine, records in data.items():
            if len(records) < 3:  # Need at least 3 months of data
                continue
            ...
            if demand_trends['recent_growth'] != 0:
                ...
                adjustment = min(max(adjustment, 0.5), 2.0)  # Limit adjustment range
            ...
            # Apply bounds and round predictions
